...
public void dial(String key)
{
	if (state == CHECK_CONNECTION)
		checkOption(key);
	else if (state == CONNECTED_LEAVE || state == CONNECTED_ENTER)
		connect(key);
	else if (state == RECORDING_OPTION1)
		record(key);
	else if (state == RECORDING_OPTION2) 
		login(key);
...
{
   if (state == RECORDING_OPTION1 || state == CHANGE_GREETING  )
	  while (voice.indexOf("#") != -1)
	  {
		  voice = voice.substring(0, voice.indexOf("#")) + voice.substring(voice.indexOf("#") + 1);
	  }
...
public void hangup()
{
   if (state == RECORDING_OPTION1 || state == RECORDING_OPTION2)
...
/*
 * First command on reset, checks whether user wants to leave
 * message or enter mailbox. Moves to connect after
 */
private void checkOption (String key) {
	if (key.equals("1")) {
		state = CONNECTED_LEAVE;
	}
	else if (key.equals("2")) {
		state = CONNECTED_ENTER;
	}
	phone.speak(FIND_MAILBOX_PROMPT);
 }
...
*/
 private void connect(String key)
 {
	 if (key.equals("#"))
	 {
		 if (accumulatedKeys.isEmpty() || accumulatedKeys.contains(key)) {
			 ...
			 currentMailbox = system.findMailbox(accumulatedKeys);
				 ...
				 {
					 if (state == CONNECTED_LEAVE) {
						 state = RECORDING_OPTION1;
						 phone.speak(currentMailbox.getGreeting());
					 }
					 else {
					 state = RECORDING_OPTION2;
...
private static final int CHECK_CONNECTION = 9;
private static final int CONNECTED_LEAVE = 1;
private static final int CONNECTED_ENTER = 8;
private static final int RECORDING_OPTION1 = 7;
private static final int RECORDING_OPTION2 = 2;
...
private static String FIND_MAILBOX_PROMPT = 
